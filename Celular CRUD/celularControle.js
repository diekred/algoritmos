...
function calcularLancamento(dataLancamento) {
    let dataDeHoje = new Date();
    const dn = new Date(dataLancamento); // transforma em Date

    const dias = Math.ceil((dataDeHoje - dn) / (1000 * 60 * 60 * 24)); // converte milissegundos em dias
    return dias;
}
function Tempo() {
    let celularMaisAntigo = ""; // Inicializa como string vazia
    let diasMaisAntigo = 0; // Inicializa em 0

    for (let i = 0; i < listaCelular.length; i++) {
        let celularAtual = listaCelular[i];
        let diasDesdeLancamento = calcularLancamento(celularAtual.dataLancamento); 
        
        if (diasDesdeLancamento > diasMaisAntigo) { // Compara corretamente agora
            diasMaisAntigo = diasDesdeLancamento;
            celularMaisAntigo = celularAtual;
        }
    }

    const outputElement = document.getElementById("divAviso"); // Certifique-se de que este ID existe
    if (celularMaisAntigo) {
        outputElement.innerHTML = "Celular mais antigo encontrado:<br>" +
            "Modelo: " + celularMaisAntigo.modelo + "<br>" +
            "Fabricante: " + celularMaisAntigo.fabricante + "<br>" +
            "Lançado há " + diasMaisAntigo + " dias.";
    } else {
        outputElement.innerHTML = "Nenhum celular encontrado na lista.";
    }
}
